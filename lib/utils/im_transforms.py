...
def GenerateSamples(roi, batch_sampler):
    found = 0
    sampled_bboxes = []
    for i in range(batch_sampler.max_trials):
        ...
        sampled_bbox = SampleBBox(batch_sampler.sampler)
...
def SampleBBox(sampler):
    ...
    aspect_ratio = max(aspect_ratio, math.pow(scale, 2.0))
    aspect_ratio = min(aspect_ratio, 1 / math.pow(scale, 2.0))
    ...
    h_off = npr.uniform(0.0, 1.0 - bbox_width)
    w_off = npr.uniform(0.0, 1.0 - bbox_height)
...
def ApplyCrop(img):
    ...
    crop_bbox[0], crop_bbox[1], crop_bbox[2], crop_bbox[3] = crop_bbox[
        1] / img_shape[1], crop_bbox[0] / img_shape[0], crop_bbox[
            3] / img_shape[1], crop_bbox[2] / img_shape[0]
...
def ApplyExpand(img):
    ...
    if prob > cfg.TRAIN.expand_prob:
        img_height = img.shape[0]
        img_width = img.shape[1]
        return img, np.array((0, 0, 1, 1), dtype=np.uint16)
